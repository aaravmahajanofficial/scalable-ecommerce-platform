...
func main() {
	...
	router.HandleFunc("GET /api/v1/carts/{id}", authMiddleware.Authenticate(cartHandler.GetCart()))
	router.HandleFunc("POST /api/v1/carts/{id}/items", authMiddleware.Authenticate(cartHandler.AddItem()))
	router.HandleFunc("PUT /api/v1/carts/{id}/items", authMiddleware.Authenticate(cartHandler.UpdateQuantity()))
